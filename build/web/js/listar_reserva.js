...
$('.boton-editar a').on('click', function () {
    var id = $(this).data('id');
    $.ajax({
        url: 'ReservaControlador?accion=editar&id=' + id,
        type: 'GET',
...
$('.boton-agregar').on('click', function () {
    $.ajax({
        url: 'ReservaControlador?accion=agregar',
        type: 'GET',
        success: function (response) {
...
function calcularCoincidencia(cadena1, cadena2, umbralMinimo) {
    if (!isNaN(Number(cadena1)) && !isNaN(Number(cadena2))) {
        // Ambas cadenas son números
        var diferencia = Math.abs(Number(cadena1) - Number(cadena2));
        var porcentaje = 1 - diferencia / Math.max(Number(cadena1), Number(cadena2));
        return porcentaje >= umbralMinimo;
    } else {
        // Las cadenas no son números, aplicar el cálculo de Levenshtein
